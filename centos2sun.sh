...
usage() {
  ...
  echo "-r Converts to Sun/OS Linux"
...
case "${old_release}" in
  centos-linux-release*);;
  rocky-release*)
    exit_message "You are already running Sun/OS Linux."
    ;;
  *)
...
if "${reinstall_all_rpms}"; then
  ...
  if [[ -n ${non_rocky_rpm[*]} ]]; then
    echo "Non-sol packages are installed. This is generally not an issue. If you see centos packages, you may need to address them and file a bug report at https://github.com/SunOS-Linux/SunOS-Linux/issues"
    printf '\t%s\n' "${non_rocky_rpm[@]}"
  fi
fi
